...
Dispatcher.prototype = {
    ...
    run: function() {
       ...
       /* if (ops[type]) {
            console.info('#green{\n[flyku info][welcome to use flyku!]\n[flyku info][Please use "flyku -h" look  usage methods!]}\n');
            return;
        }*/
        for (var type in ops) {
...
Dispatcher.handlers = {
    ...
    'generate': function() {
        ...
        console.info('begin creat project.');
        console.info('welcole to use flyku.');
    ...
    'export': function(mainpage) {
        ...
        //重新编译全部模板
        var exp = require('flyku-manage');
        if (mainpage === true) {
            mainpage = undefined;
        }
        exp.publish(mainpage);

        console.log("template compiler success!!!");

        var config = require(cmd + "/kConfig/configs");
        shell.rm('-rf', config.output); //一次性清除所有output中的中间生成文件
        shell.rm('-rf', config.header); //一次性清除所有header生成文件
