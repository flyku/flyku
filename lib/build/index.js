...
var createModule = function (mpath, suffix) {
    ...
    fs.writeFileSync(mpath + '/xxx.css', '');
    fs.writeFileSync(mpath + '/index.js', fs.readFileSync(path.resolve(__dirname, './moduleIndexJsTemplate.js')));
    fs.writeFileSync(mpath + '/m' + suffix, '<link  rel="stylesheet" href="./xxx.css" role="debug">');
    fs.writeFileSync(mpath + '/m.json', '{}');
    console.info("Template created: #green{" + path.basename(mpath) + "}");
};
